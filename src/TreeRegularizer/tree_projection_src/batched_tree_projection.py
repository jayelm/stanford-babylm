...
def get_all_hidden_states(
    ...
    process_size = 32
    ## MOD: completely separate into two different cases due to shape inconsistency
    with tqdm(total=len(input_list), disable=tqdm_disable) as progress_bar:
        while st < len(input_list):
            en = min(len(input_list), st + process_size)
            ...
            if input_masks is not None:
                ...
                if not regularizer:
                    ...
                    ## ERROR: CUDA out of memory due to long inputs
                    outputs = [model(inputs, attention_mask=mask, output_hidden_states=True).hidden_states[1:][layer_id]]
            else:
                inputs, len_mask, input_lens = tokenizer_helper(cslice)
                inputs = inputs.to(device)
            ...
            # REMEMBER: mask is 0 if the token is not attended to, and 1 if the token is attended to.
            outputs = [hs * (mask_mult) for hs in outputs]
            for idx, _ in enumerate(cslice):
                if not regularizer:
                    hidden_states = [outputs[0][idx].cpu().detach().numpy()]
                else:
                ...
                if sum_all:
                    ...
                    # Just using GPT2 tokenizer (which doesn't contain [CLS] and [EOS])
...
def get_word_vecs_from_subwords(input_list, hidden_states_all, tokenizer, pre_tokenized=None):
    def get_cumulants(hs, idx_list):
        ...
        if type(all_vecs[0]) == torch.Tensor:
...
def get_pre_tokenized_info(tokenizer, input_strs):
    """
    e.g.
        ...
        [(0, 0), (1, 1), (2, 2), (3, 4), (5, 5)]
    """
    sent_tokens = tokenizer(input_strs)["input_ids"]
    ## TODO: O(n^2)
    lwords = []
    ...
    for i in range(len(lwords)):
        ...
        for word in words:
            word_tokenized = tokenizer(word, add_special_tokens=False)["input_ids"]
            st_curr, en_curr = get_idxs(word_tokenized, sent_tokens[i], st)
            idxs.append((st_curr, en_curr))
            st = en_curr
        lidxs.append(idxs)
    return sent_tokens, lidxs
...
class TreeProjection:
    ...
    def _get_masking_info(self, input_strs):
        ...
        input_masks = []
        curr = 0
        ## MOD: split to different sentence2idx
        ## MOD: added masked_strs and input_masks according to input
