...
from tree_projection_src import tree_projection
...
class Regularizer():
    def __init__(self, model, sim_metric, tokenizer, start_relax_layer):
        ...
        self.model = model
        self.tree_projector = tree_projection.TreeProjection(self.model, self.tokenizer, sim_fn=self.sim_metric, normalize=True)

    def compute_vectors(self, input_str):
        '''
            Takes a model, input_str (string of tokens, possibly tokenized by GPT2) and returns:
            1. Contextual vectors: (num_words x hidden_dim)
            2. Context Free vectors: (num_spans x hidden_dim)
            3. span2idx: for each span, the index of the span in the context free vectors

        TODO: make this batched
        '''
        sent_tokens, idxs = tree_projection.get_pre_tokenized_info(self.tokenizer, input_str)
        sentence2idx_tuple, masked_strs, input_masks = self.tree_projector._get_masking_info([input_str])

        outer_context_vecs = tree_projection.get_all_hidden_states(
            self.model,
            self.tokenizer,
            [input_str],
            tqdm_disable=True,
        ...
        inner_context_vecs = tree_projection.get_all_hidden_states(
            self.model,
            self.tokenizer,
            masked_strs,
            input_masks,
        ...
        for idx, key in keys:
            st, en = key
            span2idx[(st, en)] = idx
        return outer_context_vecs[0][-1], inner_context_vecs, span2idx


    def __call__(self, input_str):
        contextual_vectors, context_free_vectors, span2idx = self.compute_vectors(input_str)

        num_words = len(input_str.split(' '))
        #total_score = 0.0
        return self.get_score(0, num_words-1, contextual_vectors, context_free_vectors, span2idx)
...
if __name__ == '__main__':
    ...
    for input_str in tqdm(input_strings):
        loss = reg(input_str)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
